...
const backServerTest = import.meta.env.VITE_BACK_SERVER_TEST;
...
const useNewServicetest = create((set) => ({
  ...
  fetchClaim: async (key) => {
    ...
    const res = await axios.get(
      `${backServerTest}/api/servicestest/item/${key}`
    );
    if (res.data) {
      res.data.fields = res.data.fields.map((item, index) => {
        if (item.in1C && item.name == "")
          item.name = item.in1C.Description + "_" + index;
        return item;
      });
      // console.log(data)
      set((state) => {
  ...
  createClaim: async (data) => {
    const res = await axios.post(
      `${backServerTest}/api/cabinet/claimstest`,
      data,
      {
        headers: {
          "Content-Type": "application/json",
